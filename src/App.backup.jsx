...
function Header({ role, setRole, setPage, sidebarOpen, setSidebarOpen, cloudMode }) {
  const changeRole = (nextRole) => {
    setRole(nextRole);
    setPage(nextRole === 'clipper' ? 'discover' : nextRole === 'creator' ? 'creatorCampaigns' : 'adminCampaigns');
  };
    ...
    <header className="topbar">
      ...
      <button className="brand" onClick={() => setPage('home')}>
        ...
        <div>
          ...
          <span>{cloudMode ? 'Supabase connected' : 'Local demo mode'}</span>
        </div>
      </button>
...
function AuthBox() {
  ...
  const signUp = async () => {
    ...
    setMessage(error ? error.message : 'Account created. Check email confirmation if Supabase asks for it.');
  };
  ...
  return (
    <section className="panel auth-panel">
      <div>
        <Pill tone="green"><UserRound size={14} /> Supabase Auth</Pill>
        <h2>{user ? `Logged in as ${user.email}` : 'Create or login to a SoloHub test account.'}</h2>
        <p>This is the first real backend step. Roles still use the switcher until we add profile permissions.</p>
      </div>
      {user ? (
...
function HomePage({ setRole, setPage, campaigns, submissions, cloudMode }) {
...
function App() {
  ...
  const createCampaign = async (campaign) => {
    if (cloudMode) {
      const { data, error } = await supabase.from('campaigns').insert(toCampaignDb(campaign)).select().single();
  ...
  const content = useMemo(() => {
    if (page === 'home') return <HomePage setRole={setRole} setPage={setPage} campaigns={campaigns} submissions={submissions} cloudMode={cloudMode} />;
  ...
  }, [page, campaigns, submissions, selectedCampaign, cloudMode]);

  return (
    <>
      <Header role={role} setRole={setRole} setPage={setPage} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} cloudMode={cloudMode} />
      <div className="app-shell">
        ...
        <main>
          ...
          {cloudMode && <div className="notice subtle"><span>{loading ? 'Syncing Supabase...' : 'Supabase mode active.'}</span><button onClick={loadCloudData}>Refresh cloud data</button></div>}
          {content}
        </main>
